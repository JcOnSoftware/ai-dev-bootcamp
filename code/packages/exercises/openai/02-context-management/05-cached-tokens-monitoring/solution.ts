...
export default async function run(): Promise<{
  ...
  // First call with the long system prompt
  ...
  // Second call with the exact same system prompt — OpenAI may serve it from cache
  ...
  const call1CachedTokens =
    (response1.usage as unknown as { prompt_tokens_details?: { cached_tokens?: number } })
      ?.prompt_tokens_details?.cached_tokens ?? 0;
  const call2CachedTokens =
    (response2.usage as unknown as { prompt_tokens_details?: { cached_tokens?: number } })
      ?.prompt_tokens_details?.cached_tokens ?? 0;

  return {
    call1CachedTokens,
    call2CachedTokens,
    cacheImproved: call2CachedTokens > call1CachedTokens,
  };
}
