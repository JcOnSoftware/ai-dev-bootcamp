...
/**
 * A ~700-word article spanning multiple subtopics. Embedding it as ONE vector
 * loses resolution — the result represents a blend of all three subjects.
 * Chunking into smaller pieces keeps each vector focused.
 */
...
/**
 * Split `text` into chunks of approximately `size` CHARACTERS, with a small
 * overlap so boundary sentences are preserved on both sides. A naive char
 * split is fine for this exercise — in prod you use sentence or paragraph
 * boundaries. Paragraphs in this article are separated by blank lines, so
 * splitting on "\n\n" gives reasonable chunks without manual slicing.
 */
...
/**
 * TODO:
 *   1. Chunk the ARTICLE with chunkByParagraph — should yield 4 paragraphs.
 *   2. Embed all chunks (RETRIEVAL_DOCUMENT).
 *   3. Embed the query "What disease is linked to broken mitochondria?"
 *      with RETRIEVAL_QUERY.
 *   4. Compute cosine similarity query→chunk for every chunk.
 *   5. Return them sorted by score DESCENDING with { id, text, score }.
 *      Include ALL chunks (not just top-K) so the caller can eyeball ranking.
 */
export default async function run(): Promise<Chunk[]> {
  throw new Error("TODO: chunk + embed + rank. Read exercise.md.");
}
